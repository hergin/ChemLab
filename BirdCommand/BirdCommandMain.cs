...
namespace BirdCommand
{
    ...
    public partial class BirdCommandMain : Form
    {
        ...
        private void button13_Click(object sender, EventArgs e)
        {
            ...
            start.FileName = @"Resources\WPy64-39100\python-3.9.10.amd64\python.exe";
            start.Arguments = string.Format("\"{0}\" \"{1}\"",
                @"Resources\difference-finding.py",
        ...
        private void button16_Click(object sender, EventArgs e)
        {
            ...
            start.FileName = @"Resources\WPy64-39100\python-3.9.10.amd64\python.exe";
            start.Arguments = string.Format("\"{0}\" \"{1}\"",
                @"Resources\difference-finding-just-bird.py",
                prePatternGraph.NodesToPY() + "|" + prePatternGraph.EdgesToPY() + "|" + postPatternGraph.NodesToPY() + "|" + postPatternGraph.EdgesToPY());
            start.UseShellExecute = false;// Do not use OS shell
            start.CreateNoWindow = true; // We don't need new window
            start.RedirectStandardOutput = true;// Any output, generated by application will be redirected back
            ...
            using (Process process = Process.Start(start))
            {
                using (StreamReader reader = process.StandardOutput)
                {
                    ...
                    if (string.IsNullOrEmpty(stderr))
                    {
                        ...
                        var newBirdDirectionEnum = Enum.Parse(typeof(Direction), newBirdDirectionStr);
                        ...
                        theBird.Location = new Point(theBird.Location.X + locationDifferenceX, theBird.Location.Y + locationDifferenceY);
                    }
                    else
                    {
                        MessageBox.Show(stderr, "ERROR", MessageBoxButtons.OK, MessageBoxIcon.Error);
